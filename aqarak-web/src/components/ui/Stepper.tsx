          ...
          <div className={`px-8 pb-6 ${footerClassName}`}>
            <div className={`mt-6 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className={`duration-350 rounded px-4 py-2 transition text-muted-foreground hover:text-primary`}
                  {...backButtonProps}
                >
                  {backButtonText}
                </button>
    ...
    >
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: 'hsl(var(--secondary))', color: 'hsl(var(--secondary))' },
          complete: { scale: 1, backgroundColor: 'hsl(var(--secondary))', color: 'hsl(var(--primary))' }
        }}
        transition={{ duration: 0.3 }}
        className="flex h-10 w-10 items-center justify-center rounded-full font-semibold border-2 border-border"
      >
        {status === 'complete' ? (
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: 'hsl(var(--secondary))' }
  };
